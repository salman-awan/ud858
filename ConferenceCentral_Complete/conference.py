...
# - - - Conference objects - - - - - - - - - - - - - - - - -
    ...
    def _createConferenceObject(self, request):
        """Create or update Conference object, returning ConferenceForm/request."""  # noqa
    ...
    def getConferencesCreated(self, request):
        ...
        return ConferenceForms(
            items=[self._copyConferenceToForm(conf, getattr(prof, 'displayName'))  # noqa
                   for conf in confs]
        )
    ...
    def queryConferences(self, request):
        ...
        return ConferenceForms(
                items=[self._copyConferenceToForm(conf, names[conf.organizerUserId])  # noqa
                       for conf in conferences]
        )
...
# - - - Registration - - - - - - - - - - - - - - - - - - - -
    ...
    def getConferenceSessions(self, request):
        """Return all sessions for requested conference (by websafeConferenceKey)."""  # noqa
    ...
    def getSessionsBySpeaker(self, request):
        """Return sessions given by specified speaker across all conferences."""  # noqa
    ...
    def createSession(self, request):
        """Create a new session for the requested conference."""  # noqa
        return self._createSessionObject(request)

    def _getSessionsForConference(self, websafeConferenceKey):
        """Return all sessions for the requested conference."""  # noqa
    ...
    def getWishlistSessionsByCity(self, request):
        ...
        # return SessionForm objects in the current user's wishlist
        # filtered by the given city
        return SessionForms(
                items=[self._copySessionToForm(session) for session in
                       sessions if session.key.parent().get().city ==
                       request.city])
